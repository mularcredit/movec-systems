...
export default function NetworkModuleRedirect({ tab, label }: Props) {
  ...
  if (status === 'loading') {
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-12rem)] animate-in fade-in">
        ...
        <p className="text-[14px] text-slate-500 font-medium tracking-wide">Locating active gateways...</p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-4xl mx-auto py-10 px-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="text-center mb-10">
          ...
          <h2 className="text-2xl font-semibold text-slate-800 mb-2">Select a Target Router</h2>
          <p className="text-[15px] text-slate-500 max-w-md mx-auto leading-relaxed">
            You have multiple active gateways. Please choose which router you want to inspect for <span className="font-semibold text-slate-700">{label}</span>.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {routers.map(router => (
            <div 
              ...
              className="bg-white border border-slate-200/70 p-5 rounded-2xl cursor-pointer hover:border-emerald-300 hover:shadow-lg hover:-translate-y-1 transition-all duration-200 group text-left"
            >
              <div className="flex justify-between items-start mb-4">
                <div className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center border border-slate-100 group-hover:bg-emerald-50 group-hover:border-emerald-100 transition-colors">
                  <Server className="w-5 h-5 text-slate-400 group-hover:text-emerald-500 transition-colors" />
                </div>
                {router.connection_status === 'online' ? (
                  ...
                  <span className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-50 text-slate-500 rounded-full text-[11px] font-bold tracking-wide uppercase">
                    <span className="w-1.5 h-1.5 bg-slate-400 rounded-full"></span> Offline
                  </span>
                )}
              </div>
              <h3 className="text-[16px] font-semibold text-slate-800 mb-1 line-clamp-1">{router.name}</h3>
              <p className="font-mono text-[13px] text-slate-500 mb-4">{router.ip_address}</p>
  ...
  return (
    <div className="flex flex-col items-center justify-center h-64 text-center animate-in fade-in p-10">
      ...
      <h2 className="text-[16px] font-medium text-slate-700 mb-2">No Router Linked</h2>
